...
export class Backend implements backendInterface {
    ...
    async getAllStudents(): Promise<Array<StoredStudent>> {
        if (this.processError) {
            try {
    ...
    async getClassSectionStudents(arg0: string, arg1: string): Promise<Array<[bigint, Student]>> {
        if (this.processError) {
            try {
                const result = await this.actor.getClassSectionStudents(arg0, arg1);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
